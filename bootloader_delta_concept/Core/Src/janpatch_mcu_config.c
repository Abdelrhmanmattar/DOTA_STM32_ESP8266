...
/*staitc variables to use in main
 *
 */
static janpatch_file_t source = {
  .file_address = FIRST_TARGET_ADDRESS,
  .file_size = 0x34B4,
};

static janpatch_file_t patch = {
  .file_address = PATCH_TARGET_ADDRESS,
  .file_size = 230,
};

static janpatch_file_t target = {
  .file_address = SECOND_TARGET_ADDRESS,
  .file_size = 33*1024,
};
...
void update_source(uint32_t address, uint32_t size) {
    source.file_address = address;
    source.file_size = size;
}

// Function to update the patch structure
void update_patch(uint32_t address, uint32_t size) {
    patch.file_address = address;
    patch.file_size = size;
}

// Function to update the target structure
void update_target(uint32_t address, uint32_t size) {
    target.file_address = address;
    target.file_size = 33*1024;
}
                                        ...
                                        janpatch_file_t *target)
{
    ...
    BL_SendMessage("source file size    :0x%08X\r\n", source->file_size);
    BL_SendMessage("source file crc     :0x%08X\r\n", source->file_crc32);
    BL_SendMessage("target file size    :0x%08X\r\n", target->file_size);
    BL_SendMessage("target file crc     :0x%08X\r\n", target->file_crc32);
    BL_SendMessage("patch file size     :0x%08X\r\n", patch->file_size);
    BL_SendMessage("patch file crc      :0x%08X\r\n", patch->file_crc32);
    ...
    if(source->file_address == target->file_address){
        BL_SendMessage("janpatch window disable and source address is the same as target address\r\n");
        BL_SendMessage("please enable window or change address\r\n");
        return -1;
    }
#endif
    ...
    if(val != 0) {
        BL_SendMessage("janpatch failed: %d\r\n", val);
        return val;
    }
    dev.check_crc = dev.check_crc ^ 0xFFFFFFFF;
    BL_SendMessage("check crc      :0x%08X\r\n", dev.check_crc);
    if(dev.check_crc != target->file_crc32){
        BL_SendMessage("target failed crc error: 0x%08X 0x%08X\r\n", dev.check_crc, target->file_crc32);
        return -1;
    }


    BL_SendMessage("target file size:%d\r\n", target->file_seek);

    if(target->file_seek > target->file_size){
        BL_SendMessage("target failed over addr target max size: %d\r\n", target->file_size);
        return -1;
    }
#ifdef JANPATCH_MCU_CONFIG_WINDOW_ENABLE
    if((target->file_seek - source->file_size) > JANPATCH_MCU_CONFIG_WINDOW_SIZE)
    {
        BL_SendMessage("window size is too small\r\n");
        return -1;
    }
#endif
