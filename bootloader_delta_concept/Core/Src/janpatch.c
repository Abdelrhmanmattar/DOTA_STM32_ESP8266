...
static int jp_fseek(janpatch_buffer *buffer, long int offset, int origin) {
    if (origin == SEEK_SET) {
        ...
        BL_SendMessage("Origin %d not supported in jp_fseek (only SEEK_CUR,SEEK_SET)\r\n", origin);
        return -1;
    }
    return 0;
}
...
static void process_mod(janpatch_ctx *ctx, janpatch_buffer *source, janpatch_buffer *patch, janpatch_buffer *target, bool up_source_stream) {
    ...
    while (1) {
        ...
        if (m == JANPATCH_OPERATION_ESC) {
            // BL_SendMessage("ESC, NEXT CHAR ALSO ESC\n");
            jp_putc(m, ctx, target);
            if (up_source_stream) {
                jp_fseek(source, 1, SEEK_CUR);
            }
            ...
            // BL_SendMessage("ESC, THEN OPERATOR\n");
            BL_SendMessage("%d bytes\r\n", cnt);
            jp_fseek(patch, -2, SEEK_CUR);
            break;
        }
        else { // else... write both the ESC and m
            // BL_SendMessage("ESC, BUT NO OPERATOR\n");
...
static int find_length(janpatch_ctx *ctx, janpatch_buffer *buffer) {
    ...
    */
    ...
    if (l <= 251) {
        ...
        BL_SendMessage("EQL followed by unexpected byte %02x %02x\r\n", JANPATCH_OPERATION_EQL, l);
        return -1;
    }
        ...
        if (c == JANPATCH_OPERATION_ESC) {
            switch ((c = jp_getc(&ctx, &ctx.patch_buffer))) {
                case JANPATCH_OPERATION_EQL: {
                    ...
                    if (length == -1) {
                        BL_SendMessage("EQL length invalid\r\n");
                        BL_SendMessage("Positions are, source=%ld patch=%ld new=%ld\r\n", ctx.source_buffer.position, ctx.patch_buffer.position, ctx.target_buffer.position);
                        return 1;
                    }

                    BL_SendMessage("EQL: %d bytes\r\n", length);

                    for (int ix = 0; ix < length; ix++) {
                        int r = jp_getc(&ctx, &ctx.source_buffer);
                        if (r < -1) {
                            BL_SendMessage("fread returned %d, but expected character\r\n", r);
                            BL_SendMessage("Positions are, source=%ld patch=%ld new=%ld\r\n", ctx.source_buffer.position, ctx.patch_buffer.position, ctx.target_buffer.position);
                            return 1;
                        }
                ...
                case JANPATCH_OPERATION_BKT: {
                    ...
                    int length = find_length(&ctx, &ctx.patch_buffer);
                    if (length == -1) {
                        BL_SendMessage("BKT length invalid\r\n");
                        BL_SendMessage("Positions are, source=%ld patch=%ld new=%ld\r\n", ctx.source_buffer.position, ctx.patch_buffer.position, ctx.target_buffer.position);
                        return 1;
                    }

                    BL_SendMessage("BKT: %d bytes\r\n", -length);
                ...
                case JANPATCH_OPERATION_DEL: {
                    ...
                    int length = find_length(&ctx, &ctx.patch_buffer);
                    if (length == -1) {
                        BL_SendMessage("DEL length invalid\r\n");
                        BL_SendMessage("Positions are, source=%ld patch=%ld new=%ld\r\n", ctx.source_buffer.position, ctx.patch_buffer.position, ctx.target_buffer.position);
                        return 1;
                    }

                    BL_SendMessage("DEL: %d bytes\r\n", length);
                ...
                default: {
                    BL_SendMessage("Unsupported operator %02x\r\n", c);
                    BL_SendMessage("Positions are, source=%ld patch=%ld new=%ld\r\n", ctx.source_buffer.position, ctx.patch_buffer.position, ctx.target_buffer.position);
                    return 1;
                }
            }
            ...
            BL_SendMessage("Expected ESC but got %02x\r\n", c);
            BL_SendMessage("Positions are, source=%ld patch=%ld new=%ld\r\n", ctx.source_buffer.position, ctx.patch_buffer.position, ctx.target_buffer.position);
